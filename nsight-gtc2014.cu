...
__global__ void rgba_to_grayscale_kernel(int w, int h, const uchar4 *src, uchar *dst)
{
...
#  if OPTIMIZATION_STEP == 0x00
  ...
  rgba_to_grayscale_kernel<<<grid_dim, block_dim>>>(g_data.img_w, g_data.img_h, g_data.img_rgba, grayscale);
  ...
  if( g_data.show & SHOW_SMOOTHED_GRAYSCALE )
  {
    ...
    printf("Running step: %2x", OPTIMIZATION_STEP);
#if   OPTIMIZATION_STEP == 0x00
...
int main(int argc, char **argv)
{
  ...
  {
#ifdef WITH_OPENGL
    printf("Usage: %s [-no-opengl] FILENAME.ppm\n", argv[0]);
#else
